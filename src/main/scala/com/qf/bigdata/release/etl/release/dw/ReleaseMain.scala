...
//     注册
        DWAllTheme.handleReleaseJob(spark,appName,bdp_date)
//        曝光
        DWAllTheme.exposureReleaseJobs(spark,appName,bdp_date)
//        注册
        DWAllTheme.userReleaseJob(spark,appName,bdp_date)
        //点击
        DWAllTheme.clickReleaseJob(spark,appName,bdp_date)
      }
    }catch {
      case ex:Exception=>{
